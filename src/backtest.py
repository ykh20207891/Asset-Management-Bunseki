...
def run_backtest(horizon: int, top_k: int, initial_train_days: int,
                 test_window: int, universe_ids: set | None = None,
                 listed_on: dict | None = None) -> dict:
    """universe_ids を渡すと、学習は全銘柄で行ったうえで、成績の評価
    （IC・上位K銘柄のポートフォリオ）だけをその銘柄集合に限定する。
    実際に売買できる銘柄（Bybit 上場）だけで戦略を評価するために使う。

    listed_on（coin_id -> 上場日）を渡すと、各日について「その日に既に上場していた
    銘柄」だけを評価対象にする（時点つき）。今日の上場一覧で過去を評価すると、
    上場前の急騰を「買えた」ことにしてしまうため。"""
    ...
    if listed_on:
        before = len(oos)
        listed = oos["coin_id"].map(listed_on)
        oos = oos[listed.notna() & (listed <= oos["date"].dt.strftime("%Y-%m-%d"))].copy()
        LOG.info("評価対象を『その日に上場済みの銘柄』に限定（時点つき）: %s → %s 行",
                 format(before, ","), format(len(oos), ","))
        if oos.empty:
            raise RuntimeError("時点つきで絞ると評価対象が空になりました。")
    elif universe_ids:
        before = len(oos)
